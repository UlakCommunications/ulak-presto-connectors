        ...
        implements ConnectorMetadata
{
    ...
    private UlakQuickwitMetadata(String catalogName)
    {
        connectorId = new UlakConnectorId(catalogName).toString();
    }

    public static UlakQuickwitMetadata getInstance(String catalogName)
    {
        if (single == null) {
            single = new UlakQuickwitMetadata(catalogName);
        }
        return single;
    }
...
//    }
    ...
    public ConnectorTableMetadata getTableMetadata(ConnectorSession session, ConnectorTableHandle table)
    {
        ...
        try {
            list = getColumnsBase( influxdbTableHandle.getTableName(), QwUtil.select(influxdbTableHandle.getTableName()));
        } catch (IOException e) {
    ...
    public Map<SchemaTableName, List<ColumnMetadata>> listTableColumns(ConnectorSession session, SchemaTablePrefix prefix)
    {
        ...
        for (SchemaTableName tableName : list) {
            if (tableName.getTableName().startsWith(prefix.getTable().get())) {
                try {
                    columns.put(tableName,getColumnsBase( tableName.getTableName(), QwUtil.select(tableName.getTableName())));
                } catch (IOException e) {
                    logger.error("IOException", e);
                    throw new RuntimeException(e);
                } catch (ClassNotFoundException e) {
