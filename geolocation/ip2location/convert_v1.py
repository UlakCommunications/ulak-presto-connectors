#https://raw.githubusercontent.com/fengdelan/convert-ip2location-lite-mmdb/refs/heads/master/convert.py
...
if (len(sys.argv) > 1):
    ...
    if(filename.lower().endswith('.csv')):
        ...
        for i in range(len(data)):
            ...
            for y in range(datalevel):
                ...
                if ((re.search(regex, nodedata) != None)):
                    ...
                    stringToWrite.extend(print_node(leftdata, rightdata))
        stringToWrite.extend(datastartmarker)
        stringToWrite.extend(datasection)
        stringToWrite.extend(binascii.unhexlify(b'ABCDEF4D61784D696E642E636F6D'))
        controlbyte = maptype | 9
        stringToWrite.extend(print_byte(controlbyte))
        field = "binary_format_major_version"
        fieldlength = len(field)
        controlbyte = stringtype | fieldlength
        stringToWrite.extend(print_byte(controlbyte) + bytes(field.encode()))
        myint = 2
        myint = print_uint(myint)
        intbytes = len(myint)
        controlbyte = uint16type | intbytes
        stringToWrite.extend(print_byte(controlbyte) + myint)
        ...
        typebyte = uint64type
        stringToWrite.extend(print_byte(controlbyte) + print_byte(typebyte) + myint)
        field = "database_type"
        fieldlength = len(field)
        controlbyte = stringtype | fieldlength
        stringToWrite.extend(print_byte(controlbyte) + bytes(field.encode()))
        ...
        controlbyte = extendedtype | myint
        typebyte = arraytype
        stringToWrite.extend(print_byte(controlbyte) + print_byte(typebyte))
        field = "en"
        fieldlength = len(field)
        controlbyte = stringtype | fieldlength
        stringToWrite.extend(print_byte(controlbyte) + bytes(field.encode()))
