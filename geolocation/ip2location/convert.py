...
if (len(sys.argv) > 1):
    ...
    if(filename.lower().endswith('.csv')):
        with open(filename, 'r', encoding = 'utf-8') as f:
            ...
            for row in mycsv:
                cntRead+=1
                if cntRead % 1000_000 == 0:
                    print (f"Reading csv: {cntRead} in {time.time() - end}")
                    end = time.time()
                therest = ''
                if (len(row) == 10):
                    dbtype = 'city'
        ...
        print (f"Finished Reading csv: {cntRead}  in {time.time() - start}")
        start = time.time()
        end = start
        cntOther   = 0
        for i in range(len(cidrdata)):
            ...
            cntOther+=1
            if cntRead % 1000_000 == 0:
                print (f"Reading cidr: {cntOther}/{cntRead} in {time.time() - end}")
                end = time.time()
            regex_here1 = r"^\"([\d\.]+)\/(\d+)\",(.*)"
        ...
        print (f"Finished Reading cidr: {cntOther}/{cntRead}  in {time.time() - start}")
        start = time.time()
        end = start
        cntOther   = 0
        datasection = b""
        ...
        for binarystrcidr in sorted(sortbylength):
            ...
            cntOther+=1
            if cntRead % 1000_000 == 0:
                print (f"Reading strcidr: {cntOther}/{cntRead} in {time.time() - end}")
                end = time.time()
        ...
        print (f"Finished Reading: {cntRead}  in {time.time() - start}")
        start = time.time()
        end = start
        ...
        for i in range(len(data)):
            ...
            cntWrite+=1
            if cntWrite % 1000_000 == 0 :
                print (f"Writing: {cntWrite}/{cntRead} in {time.time() - end}")
                end = time.time()
        ...
        f.write(print_byte(controlbyte) + myint)
        f.close()
        print (f"Writing: {cntWrite}/{cntRead} in {time.time() - start}")
