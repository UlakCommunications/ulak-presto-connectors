...
public class QwUtil {
    ...
    public static List<String> getSchemas() throws ApiException {
        ...
        for (VersionedIndexMetadata bucket1 : indexesMetadatas) {
            String schemaName = bucket1.getVersionedIndexMetadataOneOf().getIndexConfig().getVersionedIndexConfigOneOf()
                    .getIndexId();
            res.add(schemaName);
    ...
    // public static List<UlakRow> select(String tableName,String qwUrl, String
    // qwIndex) throws IOException, ClassNotFoundException, SQLException,
    // ApiException {
    //
    // QueryParameters queryParameters =
    // QueryParameters.getQueryParameters(tableName);
        ...
        try {
            ...
            try (Response execResp = call.execute()) {
                ...
                logger.debug("Query executed executeOneQuery: {}\n\n\nurl:{}\n\n\nindex:{}\n\n\nret size:{}",
                        ...
                        queryParameters.getQwUrl(),
                        queryParameters.getQwIndex(),
                        ret == null || ret.getAggregations() == null ? 0
                                : ((Map<String, Object>) ret.getAggregations()).size());
        ...
        for (Map.Entry<String, Object> entry : aggMap.entrySet()) {
            ...
            for (Object bucketObj : (List<?>) bucketsObj) {
                ...
                if (key instanceof Number) {
                    // Store as plain integer string, not scientific notation (e.g. "1773792000000"
                    // not "1.773792E12")
                ...
                if (keyAsString != null)
                    rowForBucket.put(aggId + "/" + KEY_AS_STRING, String.valueOf(keyAsString));

                // collect sub-aggregation maps (skip primitive metadata: key, key_as_string,
                // doc_count, etc.)
    ...
    private static List<UlakRow> trimTimeEdges(List<UlakRow> rows, QueryParameters queryParameters) {
        ...
        if (StringUtils.isBlank(timeField) || rows.isEmpty())
            return rows;
        ...
        if (maxTime == 0 || minTime == Long.MAX_VALUE)
            return rows;
    ...
    public static void arrangeAggregations(Map<String, Object> aggregation, Map<String, Object> currentValuesIn) {
        ...
        for (Map.Entry<String, Object> agg : aggregation.entrySet()) {
            ...
            if (!(aggValueObj instanceof Map)) {
                arrangeAggregation(currentValuesIn, new HashMap<String, Object>() {
                    {
                        put(aggKey, aggValueObj);
                    }
                }, aggKey);
            } else {
    ...
    private static Map<String, Object> arrangeAggregation(Map<String, Object> currentValuesIn,
            Map<String, Object> aggValue, String aggKey) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // ret = Lists.newArrayList(QwUtil.select(params,params.getQwUrl(),
            // params.getQwIndex()));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
