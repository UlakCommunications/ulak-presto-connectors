...
- [x] **J56 [P2, 1d] Plain Table Query Mode** — Phase 1 complete. `FROM quickwit.public."<index-name>"` now works as a first-class query surface. Detection: `PlainTableQuery.isPlainMode()` (no `//` in table name). Schema: `QwUtil.getColumnsFromDocMapping()` reads `DocMapping.fieldMappings` from Quickwit `IndexesApi`; Quickwit types mapped to Trino types via `fieldTypeToTrino()`. Query: `PlainTableQuery.buildMatchAllQuery()` generates `{"query":"*","max_hits":1000}` with `//qwindex=` prefix; injected in `QuickwitSplitManager.getSplits()`. Phase 2 (filter/limit pushdown via `applyFilter()`/`applyLimit()`) deferred — requires stashing `TupleDomain` on the table handle; open as J56b if needed.
